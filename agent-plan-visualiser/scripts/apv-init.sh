...
#   3. Serve launcher: generate `<data-dir>/bin/serve`, a one-word entry
#      point to the view server (delegates to the toolchain's bin/apv-serve).
#      Untracked and regenerated each run — it bakes the machine-/version-
#      specific toolchain path, exactly like the gate hooks.
#   4. Gitignore the local per-checkout state (`.last-capture`,
#      `.pending-extract`, `bin/serve`).
...
# --- 3. serve launcher -------------------------------------------------------
# A one-word entry point to the view server, so nobody has to type
# `python3 <toolchain>/scripts/serve.py`. Generated (never hand-authored),
# untracked (like the gate hooks, it bakes the machine- and version-specific
# toolchain path), regenerated on every init so a plugin update that moves
# the toolchain home is picked up by a re-run. Delegates to bin/apv-serve,
# the single implementation. Run it as `$DATA_DIR/bin/serve` from the repo
# root.
SERVE_PATH="$DATA_DIR/bin/serve"
SERVE_WANT="$(cat <<SERVE
#!/usr/bin/env bash
# $SERVE_PATH — local APV view-server launcher.
# GENERATED by apv-init; UNTRACKED and regenerated on re-init. Do not commit
# or hand-edit: the toolchain path below is machine- and version-specific.
# After a plugin update, re-run /apv-init to refresh it.
exec "$TOOLCHAIN_HOME/bin/apv-serve" "\$@"
SERVE
)"
if [ -f "$SERVE_PATH" ] && [ "$(cat "$SERVE_PATH")" = "$SERVE_WANT" ]; then
  report ok "$SERVE_PATH" "run it to open the view UI"
else
  had_serve=0; [ -f "$SERVE_PATH" ] && had_serve=1
  mkdir -p "$DATA_DIR/bin"
  printf '%s\n' "$SERVE_WANT" > "$SERVE_PATH"
  chmod +x "$SERVE_PATH"
  if [ "$had_serve" -eq 1 ]; then
    report updated "$SERVE_PATH" "toolchain path refreshed"
  else
    report created "$SERVE_PATH" "run it to open the view UI"
  fi
fi

# --- 4. gitignore the local capture state + serve launcher -------------------
# The stamp (guard), the pending-extract flag (extractor's post-commit half),
# and the generated serve launcher are per-checkout local state; never
# tracked. One line each, appended once.
IGNORED_ALL=1
for LOCAL_LINE in "$DATA_DIR/.last-capture" "$DATA_DIR/.pending-extract" "$SERVE_PATH"; do
...
if [ "$IGNORED_ALL" -eq 1 ]; then
  ...
  report created ".gitignore" "ignoring .last-capture + .pending-extract + bin/serve"
fi
...
if [ "$FAIL" -eq 0 ]; then
  ...
  echo "  - See the UI: run ./$SERVE_PATH (then open the printed view URL)."
