...
#      with 0.3.0 (/apv-capture seals quote the commit's first line);
...
# Resolve the data dir (env -> .apv-config.toml -> default) through the same
# apvlib the rest of the toolchain uses, plus its repo-relative path (needed
...
import apvlib
d = apvlib.apv_data_dir(root, cfg).resolve()
try:
