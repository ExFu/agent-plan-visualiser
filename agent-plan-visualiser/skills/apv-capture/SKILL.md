---
name: apv-capture
description: Capture the current session's work as structured events in the agent-plan-visualiser event log. Use after completing a logical unit of work, immediately before git-committing, in any project tracked by agent-plan-visualiser (has a .agent-plan-tracker/events.jsonl or APV_DATA_DIR equivalent).
---

# /apv-capture — record what just happened as events
...
5. **Sub-project attribution** (repos with a `[projects]` registry; T3-project-attribution): new entities are attributed at creation, by location — **only named sub-projects are ever stamped**, the default project never is. A new plan under a *named* project's planning root ⇒ `attributes.project: "<name>"` on its `entity.created`. Planless entities (inbox-item, blocker, hitl-question, implicit-work) whose work falls in a named project's carve-outs (`dirs` + its planning root) ⇒ same stamp — you have session context; you know the home. Work under no carve-out gets **no stamp** (it folds to `main`, or `unassigned` for planless entities — the visible triage floor). Work spanning **several** named sub-projects splits into one entity per project (ruling 2026-07-22: no multi-project entities). Deliberate re-homing later is `project.assigned` (§2) — operator-ruled, never a side effect.
