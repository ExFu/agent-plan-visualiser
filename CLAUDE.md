...
The T1 plan remains in active authoring as the design source of truth. **M4 (fresh-install packaging) is in build** — planned and accepted 2026-06-10 (`planning/M4-fresh-install.md`, five T3s): `T3-toolchain-portability` (the APV rename + toolchain-home resolution, installers bake paths via `--home=`) is built; init flow (`.apv/` data dir for fresh installs), session orientation, the exfu.ai test-channel bundle, and the autonomous `claude -p` extractor (in-wave by operator ruling) follow. M5 (backfill) after.
