...
- **The full autonomous `claude -p` extractor** for non-Claude-Code committers — scheduled as M4's **later wave** (§5), authored after the install wave ships; whether it stays in M4 or slips to its own milestone is an acceptance-time operator call (§7 Q1).
...
## 5. How M4 delivers — T3 tasks (install wave)

All under T2-packaging. Consolidates that plan's §5 M4-scheduled candidates (`T3-npm-package-config` + `T3-cowork-compat-verify` → #4; `T3-cli-install-command` + `T3-target-project-init-flow` → #2).
...
2. **`T3-project-init-flow`** [depends #1] — the init command: seed, config, hooks, CLAUDE.md offer, idempotent re-run, existing-repo handling.
...
4. **`T3-distribution`** [depends #1–3] — npm vs plugin-marketplace decision + artefact; Cowork compatibility verification; the CI gate-adapter template (M3's deferral lands); README quickstart.

**Later wave** (authored after the install wave ships): `T3-autonomous-extractor` — the `claude -p` per-commit extractor for non-Claude-Code committers.
