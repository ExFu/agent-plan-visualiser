...
**Status**: Draft.
**Sits at**: T2-extraction theme, M2-auto-extract milestone. M2 headline deliverable — parallel to `T3-configurable-data-dir`.
...
2. **Ontology summary at schema `0.2.0`.** A concise reference the agent consults during capture:
   - The 25 event types (9 entity lifecycle + 1 decision + 3 blocker + 4 verification + 5 relationship + 1 meta + 2 analysis) with one-line descriptions and required attributes.
...
4. **Event-emission rules.**
   - Every event needs: `event_id` (UUID v4), `type`, `entity_type`, `entity_id`, `actor`, `confidence` (`explicit` | `derived`), `schema_version` (`"0.2.0"`), `attributes`.
...
5. **The `.last-capture` timestamp.** After appending all events, write the current timestamp to `.agent-plan-tracker/.last-capture` (or the configured `APT_DATA_DIR` equivalent). This file is gitignored and untracked — it's local state consumed by the capture-guard hook.
...
The skill references the schema files (`schemas/0.2.0/events.schema.json`, `schemas/0.2.0/plan-frontmatter.schema.json`) for the agent to consult if it needs precise field validation.
...
- T2-ontology — the `0.2.0` schema the skill references.
