...
# T1-top-level — agent-plan-visualiser — Top-level plan

**Status**: Active authoring (design SSOT). Still edited as the design evolves; rarely superseded.
**Phase**: implementation underway. **M1 (hand-rollable end-to-end) is complete**, plus M6-analyser (all phases) and M1.2-relationship-ssot. **M2 (skill-driven per-commit capture) is complete** — `/apv-capture` is the canonical producer, the capture-guard hook is live, and the draft→accepted lifecycle (schema `0.3.0`) gates implementation work. **M3 (clean gate) is complete** (2026-06-10) — `gate-check` (integrity composite + seal↔commit correspondence, policy in `.apv-config.toml`) fires through three adapters (`/apv-merge` procedural, pre-push, reference-transaction local), and the milestone's own delivery landed on main via its doctrine, gate green. The tracker runs green against this project via `repack-validate.sh`. Next frontier: M4 (fresh-install packaging). Several §5 questions are now closed (notably Q9 — milestone membership is event-sourced); the rest resolve through M4+ dogfooding.
...
So the only legal spawn-parents of a milestone are: a Tier-1 plan, a top-level side-quest head, or (for a sub-milestone) its parent milestone. The dotted `Mn.p` scheme is the notation for sub-milestones; `M1.1-analyser` is the first one in use (renumbered from the historical `M6-analyser` on 2026-06-08).

> **On grandfathering vs renumbering (updated 2026-06-08).** Two tools exist for a milestone filed in the wrong place. **Reattach** (`relationship.reattached`, see [[T2-ontology]] §3.6) moves a milestone to its true *parent* while keeping its *label* — cheap, no id churn, used when the label is acceptable. **Rename** (`entity.renamed` identity migration, see [[T2-ontology]] §3.11) changes the milestone's *canonical id* itself — the heavier move, used when the label is genuinely wrong. The analyser used reattach first (M1.2, when `entity.renamed` was still inert) and rename later (2026-06-08, once the identity-migration fold was built): `M6-analyser` → `M1.1-analyser`. So flat indices are *no longer* unconditionally grandfathered — a mis-numbered milestone can now be properly renumbered when the correctness gain is worth the churn.
...
9. ~~**Milestone-membership representation.** Currently proposed as T3 frontmatter (`milestone: M1-bootstrap`), not a graph relationship event. Rationale: scheduling adjusts naturally as work shapes up, and capturing every reshuffle as an event would be noise. Confirm during M1 dogfooding that frontmatter-only is sufficient for the milestone-progress projection — if not, promote to a tracked relationship.~~
   ...
   **RESOLVED 2026-06-01 (M1 close-out dogfooding) — frontmatter-only is insufficient; the event log is the single source of truth; `relationship.reattached` is the move primitive.** The "frontmatter-only" rationale broke the moment we tried to use it. Re-tagging the mis-filed analyser T3s out of M1-bootstrap exposed that milestone membership is read from **four** inconsistent places: the `entity.created` snapshot frozen in the cache (`entities.attributes`), the frontmatter-derived graph edge synthesised off that frozen snapshot, the *live* `planning/*.md` frontmatter read at projection time in `compute_milestone_progress()`, and the `relationship.reattached` event — which was dead code (the edge-builder reads `from_entity_id`, but reattach carries `from_parent`/`to_parent`, so reattaches were silently skipped). Editing a frontmatter field moved the live-read projection but not the frozen cache or the graph edge — reproducing the divergent-source-of-truth failure §3.1/§3.2 exist to prevent, *inside the tracker itself*. **Resolution:** parentage (thematic `t2_parent` and `milestone`) is a *projection* of the event log. Frontmatter `t2_parent`/`milestone` is demoted to a creation-time declaration that **seeds** the first edge via the `entity.created` event; every subsequent move is a `relationship.reattached` event carrying an `axis` discriminator (`t2-parent` | `milestone`), which rewrites the edge in the cache and shows the reshuffle in the graph. The original "would create noise" worry (see §2.4) is overridden: a milestone move is a real event worth recording, and moves are infrequent. Frontmatter-vs-event drift becomes possible and is caught by an M3 audit rather than being structurally impossible. Implementation tracked in `T3-event-sourced-relationships` (filed under T2-storage); the analyser re-tag is its first acceptance test. This also requires correcting T2-storage §3.8, which currently asserts the opposite ("the frontmatter field IS the source of truth … no drift risk").
