...
## 13. Release 0.8.2 (2026-09-03) — command/skill name collision removed. **No re-attach required.**

Patch bump: two files deleted from the shipped tree, nothing on the capture, gate or extraction paths changes. `[requires] apv_min_version` stays `0.6.4`.

**What the cut carries**

- **fix:** `commands/apv-capture.md` and `commands/apv-merge.md` are deleted. Each shared its base name with the skill of the same name (`skills/apv-capture/`, `skills/apv-merge/`), and Claude Code surfaces plugin commands and skills in one slash namespace — so `/apv-capture` and `/apv-merge` resolved to the skill and the command file was dead weight. The `exfu-marketplace` catalogue's `scripts/check` (its CI check on every push) fails the entry outright on that collision, which is what surfaced it. `commands/apv-init.md` and `commands/apv-triage-why.md` have no skill twin and stay.

**What was verified before deleting.** `claude --debug` on 2.1.237 logs `Loaded 4 commands from plugin exfu-agent-plan-visualiser` with no warning line, so the "warns on every load" claim in the field report was not reproduced on this build; the collision itself is real regardless — one name, two definitions, the skill wins. Nothing user-facing is lost: the skill remains invocable as `/apv-capture` (and plugin-namespaced as `/exfu-agent-plan-visualiser:apv-capture`), every self-check already asserts on the skill and never the command ([[T3-cross-client-install]] ruling), and the dist sandbox's structural checks assert on the skills plus `commands/apv-init.md` only. After the deletion: `gate-check` PASS, `audit-toolchain-paths` PASS, `claude plugin validate --strict` PASS, dist sandbox ALL PASS.
