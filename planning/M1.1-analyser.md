...
# M1.1-analyser — On-demand "what's outstanding?" analyser, rolled out across five phases

> **Renamed 2026-06-08 — was `M6-analyser`.** This milestone was originally (mis)labelled `M6-analyser`. It was always conceptually the *first sub-milestone of M1* — analyser capability built on top of the M1 bootstrap, prioritised ahead of M2–M5. During M1.2 the operator chose to grandfather the flat `M6` label and merely **reparent** it to `M1-bootstrap`; that call was **reversed** here and the milestone renumbered to `M1.1-analyser` via an event-sourced `entity.renamed` (the identity-migration fold in `cache-build.py`). The old `M6` label now survives only in the historical record — the rename event's `from_name`, and the completed-plan narratives in `M1.2-relationship-ssot`, `T3-event-sourced-relationships`, and `T3-milestone-parent-ontology`. References to "M6" / "sixth milestone" below are left as-authored history.

**Status**: Completed (all five phases A–E delivered). Originally authored as "planned".
**Sits at**: First **sub-milestone of M1** on the sequence axis (`milestone_index` 1.1; hangs off `M1-bootstrap`). Touches one theme: T2-analyser. Indirectly extends T2-ontology (new event types), T2-storage (summary files), T2-projection (flow-view rendering, sidebar surfaces).

**Position in M-axis ordering note (historical):** Bootstrap delivered (M1). Auto-extract (M2), cleanliness gates (M3), packaging (M4), ingest (M5) are queued but no work has begun. The analyser was prioritised ahead of M2–M5 because it is immediately useful against the project's own growing event log — operator demand was real and present, while M2–M5 deliver value only at scale. (Originally framed as "M6 prioritised ahead of M2–M5"; it is properly an M1 sub-milestone, hence the M1.1 renumber.)
