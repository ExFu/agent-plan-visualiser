-- agent-plan-visualiser cache schema v0.6.0
-- (no DDL change from v0.5.0 — the 0.6.0 epoch adds the project.assigned
--  event type only (state-neutral sub-project membership assertion,
--  T3-retrospective-project-annotation); see
--  schemas/0.6.0/events.schema.json. The event feeds the existing
--  entities.project membership fold; no new column.)
-- Prior-epoch note kept for the record: the 0.5.0 epoch added
-- verification.deferred; within 0.5.0, T3-multi-project added
-- entities.project — the multi-project membership fold (cache-only diff;
-- the cache is dropped and rebuilt every run, so the column self-deploys).
