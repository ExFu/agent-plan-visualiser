...
- **Verdicts** (audit/review mode): summary text, not new event types — an `entity.extended` on the audited plan (or the plan owning the reviewed work) whose summary states delegate, model, verdict, and finding counts. Keep the full verdict JSON in `.exfu/returns/` (disposable); the summary is the durable record. Write that summary for a human reader under the humane-summaries peer convention (apv-capture §0.5: `exfu-humane-agents:exfu-summarising` when loaded, else its fallback) — the delegate, model and counts are fine as trailing facts, but the sentence leads with what the audit found and what it means.
