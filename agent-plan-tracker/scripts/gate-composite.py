...
Check lists come from `.apt-config.toml` `[gate]` — moving an id between
...
0.3.0 capture discipline (apt-capture SKILL §3/§4: created-first, draft
...
# Built-in defaults — identical to the committed .apt-config.toml [gate]
    ...
    def cache(self):
        ...
        if cache_path.exists():
            try:
                ...
                stale = True
        if stale:
            env = dict(os.environ, APT_DATA_DIR=str(self.data_dir))
            proc = subprocess.run(
...
def main():
    ...
    ap.add_argument("--config", type=Path, default=None,
                    help="path to .apt-config.toml (default: <repo-root>/.apt-config.toml; "
                         "explicit path must exist)")
    ap.add_argument("--data-dir", type=Path, default=None,
                    help="data dir override (default: aptlib resolution — env, config, default)")
    ...
    # APT_DATA_DIR into the cache-build subprocess (different cwd), where a
    ...
    if args.config and not args.config.exists():
        ...
        return 2
    try:
        cfg = aptlib.apt_config(repo_root, args.config)
    ...
    data_dir = args.data_dir or aptlib.apt_data_dir(repo_root, args.config)
