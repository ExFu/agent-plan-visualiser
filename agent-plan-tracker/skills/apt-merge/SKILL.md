---
name: apt-merge
description: Land a branch on main without corrupting the agent-plan-tracker event log. Use when a branch is ready for main in any tracked project (has a .agent-plan-tracker/events.jsonl or APT_DATA_DIR equivalent), and especially when git reports a merge conflict on events.jsonl — that conflict is this skill's designed trigger, never a nuisance to auto-resolve.
---

# /apt-merge — bring a branch to main, record intact
...
- Branch work is committed and captured (/apt-capture): the log ends in a seal, nothing staged.
- `bash agent-plan-tracker/scripts/gate-check.sh` is green on the branch **as it stands** — repair first; never carry defects into a reconciliation.
- `DATA_DIR` resolves as /apt-capture §0; `DATA_REL` below is its repo-relative path (default `.agent-plan-tracker`). Snippets assume the repo root as cwd.
...
The capture guard firing on a conflicted merge is **correct behaviour** — reconciliation is capturable work. Append one block per /apt-capture, ending in a `commit.recorded` seal whose `message_first_line` exactly matches the merge commit message you are about to use:
...
```bash
bash agent-plan-tracker/scripts/gate-check.sh
```
...
3. Read both tails. Same entity closed on one side and worked on the other → decide explicitly; record the ruling as `entity.reopened` + `decision` appended after (shapes: /apt-capture §2).
...
5. `bash agent-plan-tracker/scripts/gate-check.sh` — fix until green, then `git checkout main && git merge --ff-only <branch>`.
